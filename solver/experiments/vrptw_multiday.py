...
def create_data_model():
    """
    Depot + 5 clienti, axa de timp pe 2 zile:
    joi = 540-1290 (9:00-21:30), vineri = joi+1440 = 1980-2730
    """
    data = {}
    data["time_matrix"] = [
        [0, 8, 12, 6, 10, 7],
    ...
    data["time_windows"] = [
        [(540, 2730)],                        # 0 depot - "deschis" toata perioada (nu conteaza, e Start/End separat)
        [(540, 1290)],                        # 1 doar JOI, toata ziua
        [(1980, 2730)],                       # 2 doar VINERI, toata ziua
        [(540, 1290), (1980, 2730)],          # 3 JOI SAU VINERI - gemenii!
        [(540, 1290), (1980, 2730)],          # 4 "oricand" - practic identic cu 3 aici (ambele zile complet deschise)
        [(2000, 2060)],                       # 5 fereastra STRANSA, doar vineri dimineata devreme
    ]
    data["num_vehicles"] = 2   # vehiculul 0 = joi, vehiculul 1 = vineri
    data["depot"] = 0
    return data

vehicle_windows = [
    (540, 1290),    # vehicul 0 = joi
    (1980, 2730),   # vehicul 1 = vineri
]
...
def main():
    ...
    routing.AddDimension(
        ...
        2730,   # slack: poate astepta oricat (intre zile e o "asteptare" uriasa)
        2730,   # orizontul complet: doua zile
        False,
        time,
    )
    ...
    for vehicle_id in range(data["num_vehicles"]):
        start, end = vehicle_windows[vehicle_id]
        time_dimension.CumulVar(routing.Start(vehicle_id)).SetRange(start, end)
        time_dimension.CumulVar(routing.End(vehicle_id)).SetRange(start, end)
